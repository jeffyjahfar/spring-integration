...
public class AnyPoolCachingSessionFactory<F> implements SessionFactory<F>, DisposableBean {
	...
	private static final Log LOGGER = LogFactory.getLog(AnyPoolCachingSessionFactory.class);

	private final SessionFactory<F> sessionFactory;

	private final Pool<Session> pool;
 ...
 */
public AnyPoolCachingSessionFactory(SessionFactory<F> sessionFactory, Pool<Session> pool) {
	...
	public class CachedSession implements Session<F> { //NOSONAR must be final, but can't for mocking in tests
		...
		public synchronized void close() {
			if (this.released) {
				...
				if (this.sharedSessionEpoch != AnyPoolCachingSessionFactory.this.sharedSessionEpoch) {
					if (LOGGER.isDebugEnabled()) {
				...
				AnyPoolCachingSessionFactory.this.pool.releaseItem(this.targetSession);
				this.released = true;
			}
		}
